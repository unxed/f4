...
func TestQuickViewSelectionAndDirectoryRenderingContracts(t *testing.T) {
	...
	if !strings.Contains(joined, "demo") || !strings.Contains(joined, "150%") || !strings.Contains(joined, "4096") || !strings.Contains(joined, "Scanning") {
	...
	joined = strings.Join(lines, "\n")
	if !strings.Contains(joined, "broken") || !strings.Contains(joined, "walk failed") || strings.Contains(joined, "Scanning") {
