...
// f4 #1415 (part 2): a hard word-wrap break (no spaces to land on) consumes
// exactly as many columns as the wrap engine is told it has, every time --
// with no column reserved for it, the mark would have nowhere to go on
// EVERY such row, not just occasionally. A long run of a single repeated
// character forces this hard-wrap path deterministically.
func TestEditorView_WordWrapReservesColumnForMarkOnHardBreak(t *testing.T) {
	theme.SetDefaultF4Palette()
	vtui.FrameManager.Init(vtui.NewSilentScreenBuf())

	// Same geometry as TestEditorView_WordWrapDrawsWrapMark: X1=0, X2=7,
	// scrollbar gutter takes 1 of the 8 raw columns, so the text area is 7
	// columns wide. No spaces anywhere, so every wrap point is a hard break.
	Pt := piecetable.New([]byte("bbbbbbbbbbbbbbbbbbbbbbbbbbbbbb")) // 30 b's
	ev := NewEditorView(Pt, nil, "")
	defer ev.Close()
	ev.WordWrap = true
	ev.SetPosition(0, 0, 7, 10)
	...
	frags := ev.Engine.GetFragments(0)
	if len(frags) < 3 {
		t.Fatalf("expected at least 3 fragments from 30 hard-wrapped columns at width 7, got %d", len(frags))
	}
	...
	wrapMark := vtui.Palette[theme.ColEditorWrapMark]
	for row := 0; row < len(frags)-1; row++ { // every fragment except the line's real last one
		cell := scr.GetCell(lastTextCol, firstTextRow+row)
		if rune(cell.Char) != '»' || cell.Attributes != wrapMark { // #nosec G115 -- Char holds a rendered rune, well within int32 range.
			t.Errorf("hard-wrapped row %d last cell = %q attr %x, want '»' in ColEditorWrapMark (%x)", row, rune(cell.Char), cell.Attributes, wrapMark) // #nosec G115 -- same as above.
		}
	}
}
