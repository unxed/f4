...
EditorColorerScheme|syntax|Colorer|Color style|Select the color style used for syntax, guide and editor base colors. Empty uses the built-in default.||Colorer reload
...
EditorColorerBackground|syntax|Colorer|Use Colorer base colors|Use foreground and background fields supplied by the color style instead of only the general editor palette.||Colorer reload
...
EditorColorerPairs|syntax|Colorer|Highlight pairs|Draw the paired token under the cursor, such as a bracket, and its match when it is on screen, in the colors the color style gives pairs.||live
