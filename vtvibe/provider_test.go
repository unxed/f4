...
func TestConfigChat(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		io.WriteString(w, `{"choices":[{"message":{"content":[{"type":"text","text":"first "},{"type":"text","text":"second"}]}}],"usage":{"prompt_tokens":3,"completion_tokens":5}}`)
	}))
	defer server.Close()
...
func TestConfigModels(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		io.WriteString(w, `{"data":[{"id":"models/gemini"},{"id":"plain"}]}`)
	}))
	defer server.Close()
