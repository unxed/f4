...
*   **Step 17 — Support for Windows hosts.** Windows machines as remote FISH+ servers, reached over `sshd`'s own OpenSSH-Server with `DefaultShell` set to `powershell.exe`. `helper.ps1` speaks the same wire protocol v1 as `helper.sh` (`plugins/netfox/fishplus/WINDOWS_PORT.md` has the command-by-command PowerShell mapping and the POSIX-shaped-path convention it uses on the wire — `/c/Users/foo` for `C:\Users\foo`, one virtual-root entry per drive letter). `script.go` adds `HelperSourcePwsh`/`HelperScriptPwsh`/`Base64BootstrapLinePwsh` alongside the POSIX helper's own; `session.go` gains the `BootstrapBase64LinePwsh` bootstrap method and a `flavor:pwsh` banner tag read back through `Features.Flavor()`. Flavor detection did not need a probe round trip: `NewFishVFS` (the production entry point every panel opens through) tries the POSIX bootstrap first and falls back to the PowerShell one only on a handshake failure (`fish_vfs.go`'s `NewFishVFSOnDialers`/`establishWithFallback`) — zero overhead on the common Linux/BSD case, one extra round trip only against an actual Windows peer, and the working flavor is promoted to primary so every reconnect after the first goes straight to it. `session_pwsh_test.go`/`script_pwsh_test.go` run the real helper against a local `pwsh`, gated on it being on `PATH`. Running `f4` itself as a native Windows FISH+ server (this step's other original idea) stays open — nothing here forecloses it, it just was not the path taken.
...
*   **Step 20a — SFTP's own random access.** SFTP already had the advantage Step 20 asks for: `SFTPVFS.Open` in `plugins/netfox/sftp_vfs.go` returns a `sftpFileWrapper` whose `ReadAt` forwards straight to `*sftp.File.ReadAt` in the `github.com/pkg/sftp` client, which answers with `SSH_FXP_READ` at the requested byte offset — one round trip, nothing downloaded before it, and no shell in the loop the way FISH+'s own `read <offset> <length>` needs one. That has been true since SFTP support first landed, well before FISH+ existed, so no production code changed here. What this step adds is proof: `TestSFTPFileReadAtIsRandomAccessNotFullDownload` in `plugins/netfox/sftp_readat_test.go` runs a real `pkg/sftp` client against a real `pkg/sftp` request server (its own in-memory-backed test filesystem) over a TCP loopback connection, uploads an 8MB file, reads a 4KB window near its end, and asserts the bytes crossing the wire for that read stay within a small multiple of 4KB rather than anywhere near 8MB — the same property `TestFileReadAtAndCache` checks for FISH+, aimed at SFTP's own protocol instead of the shell helper's. SCP is a stream-oriented protocol with no offset in its wire format and has no VFS backend in f4 yet, so it stays under Step 20 below until one exists.
...
*   **Step 20 — FISH+ features for SCP.** SFTP's half of this moved to Step 20a above. SCP itself is stream-oriented and has no offset in its own wire format, so a random-access `vfs.ReadAt` for it, if it is worth having at all once a SCP-backed VFS exists, will need either a range restart per read (one new transfer per seek, which is what plain `scp` offers) or a different trick entirely; there is nothing to extend yet because SCP is not a client-side VFS backend in f4 today.
